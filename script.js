...
short: null,
long: null,
zoomLevel: 1 // Initial zoom level for the player
},
creatorPage: {
currentLongVideo: {
id: null,
...
card.innerHTML = <div class="long-video-thumbnail" style="background-image: url('${escapeHTML(thumbnailUrl)}')" onclick="playYouTubeVideoFromCard('${videoId}')"> <i class="fas fa-play play-icon-overlay"></i> </div> <div class="long-video-info-container"> <div class="long-video-details" onclick="navigateTo('creator-page-screen', { creatorId: '${video.snippet.channelId}', startWith: 'home' })"> <span class="long-video-name">${escapeHTML(video.snippet.title)}</span> <span class="long-video-uploader">${escapeHTML(video.snippet.channelTitle)}</span> </div> </div> <button class="add-channel-btn-long-grid haptic-trigger" onclick="${addChannelOnClick}"><i class="fas fa-plus"></i> Add Channel</button>;
return card;
...
document.getElementById('player-zoom-in-btn').addEventListener('click', () => zoomCreatorPlayer(1.1));
document.getElementById('player-zoom-out-btn').addEventListener('click', () => zoomCreatorPlayer(0.9));
}

function zoomCreatorPlayer(factor) {
const playerWrapper = document.querySelector('#creator-page-player-container .player-wrapper');
if (!playerWrapper) return;
appState.creatorPagePlayers.zoomLevel *= factor;
playerWrapper.style.transform = scale(${appState.creatorPagePlayers.zoomLevel});
}
...
appState.creatorPagePlayers.zoomLevel = 1; // Reset zoom on new video
...
if (isEligible) {
contentHTML = <div class="reward-status-container"> <h2>Congratulations!</h2> <p>You have earned a new scratch card.</p> <div class="scratch-card-container" id="scratch-card-container"> <div class="scratch-card-overlay" id="scratch-overlay">Scratch Here</div> <div class="scratch-card-reward" id="scratch-reward"></div> </div> </div>;
} else {
const timeToShow = new Date(secondsRemaining * 1000).toISOString().substr(11, 8);
contentHTML = <div class="reward-status-container"> <h2>Next Reward</h2> <p>You can earn a new reward after the timer ends.</p> <div class="reward-timer" id="reward-timer-display">${timeToShow}</div> </div>;
...
const referringUserDoc = querySnapshot.docs;
...
const message = Shubhzone Withdrawal Request: -------------------------- User UID: ${appState.currentUser.uid} Amount: ₹${amount.toFixed(2)} UPI ID: ${upiId} Mobile No: ${mobileNo} --------------------------;
