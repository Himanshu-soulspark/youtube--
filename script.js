...
databaseURL: "httpshttps://shubhzone-4a6b0-default-rtdb.firebaseio.com",
...
case 'monetag_directlink':
     ...
     const newWindow = window.open('httpshttps://otieu.com/4/9578561', '_blank');
 ...
 case 'monetag_popunder':
     ...
     monetagPopunderScript.text = `(s=>{s.dataset.zone='9578563',s.src='httpshttps://al5sm.com/tag.min.js'})([document.documentElement, document.body].filter(Boolean).pop().appendChild(document.createElement('script')))`;
...
uid: null,
username: "new_user",
avatar: "httpshttps://via.placeholder.com/120/222/FFFFFF?text=+",
email: "",
name: "",
mobile: "",
address: "",
...
playerScale: 1, // ★ नया: ज़ूम कार्यक्षमता के लिए प्लेयर स्केल को ट्रैक करता है
},
adState: {
timers: {
...
if (doc.exists) {
    ...
    const initialData = {
        ...
        name: '',
        email: user.email || '',
        avatar: user.photoURL || 'httpshttps://via.placeholder.com/120/222/FFFFFF?text=+',
...
if (profileHeaderAvatar) profileHeaderAvatar.src = appState.currentUser.avatar || "httpshttps://via.placeholder.com/50/222/FFFFFF?text=+";

profileImagePreview.src = appState.currentUser.avatar || "httpshttps://via.placeholder.com/120/222/FFFFFF?text=+";
...
videos.forEach((video) => {
    ...
    const uploaderAvatar = 'httpshttps://via.placeholder.com/40';
...
// ★★★ बदला हुआ फंक्शन: चैनल के नाम से लंबे वीडियो खोजने के लिए लॉजिक में सुधार किया गया ★★★
async function performLongVideoSearch() {
    const input = document.getElementById('long-video-search-input');
    let query = input.value.trim();
    if (!query) return;

    // 1. जांचें कि क्या इनपुट एक सीधा YouTube URL/ID है
    ...
    // 2. पहले क्वेरी के साथ एक चैनल खोजने का प्रयास करें
    try {
        console.log(`[Search] Attempting to find channel for query: "${query}"`);
        const channelSearchData = await fetchFromYouTubeAPI('search', { q: query, type: 'channel', limit: 1 });

        if (channelSearchData && channelSearchData.items && channelSearchData.items.length > 0) {
            const channelId = channelSearchData.items[0].id.channelId;
            console.log(`[Search] Channel found: ${channelSearchData.items[0].snippet.title} (ID: ${channelId}). Fetching its long videos.`);
            // 3. यदि चैनल मिल गया, तो उसके लंबे वीडियो प्राप्त करें
            finalData = await fetchFromYouTubeAPI('search', {
                channelId: channelId,
                order: 'date', // चैनल पेज के लिए अधिक प्रासंगिक
                videoDuration: 'long',
                type: 'video'
            });
        } else {
            // 4. यदि कोई चैनल नहीं मिला, तो एक सामान्य वीडियो खोज करें
            console.log(`[Search] No specific channel found. Performing general search for long videos.`);
            finalData = await fetchFromYouTubeAPI('search', {
                q: query,
                videoDuration: 'long',
                type: 'video'
         ...
         // त्रुटि होने पर सामान्य खोज पर वापस जाएं
         finalData = await fetchFromYouTubeAPI('search', {
            q: query,
            videoDuration: 'long',
            type: 'video'
        ...
        return `
            <div class="holographic-card">
                <div class="profile-pic" onclick="showEnlargedImage('${user.avatar}')">
                    <img src="${user.avatar || 'httpshttps://via.placeholder.com/50'}" alt="Profile Pic">
                </div>
                <div class="info">
...
// ★★★ नया फंक्शन: प्लेयर को ज़ूम इन करने के लिए ★★★
function zoomInPlayer() {
    const playerWrapper = document.querySelector('#creator-page-player-container .player-wrapper');
    if (!playerWrapper) return;
    // अधिकतम ज़ूम 2x तक सीमित करें
    if (appState.creatorPage.playerScale >= 2.0) return;
    appState.creatorPage.playerScale += 0.1;
    playerWrapper.style.transform = `scale(${appState.creatorPage.playerScale})`;
}

// ★★★ नया फंक्शन: प्लेयर को ज़ूम आउट करने के लिए ★★★
function zoomOutPlayer() {
    const playerWrapper = document.querySelector('#creator-page-player-container .player-wrapper');
    if (!playerWrapper) return;
    // न्यूनतम ज़ूम 0.5x तक सीमित करें
    if (appState.creatorPage.playerScale <= 0.5) return;
    appState.creatorPage.playerScale -= 0.1;
    playerWrapper.style.transform = `scale(${appState.creatorPage.playerScale})`;
}


// ★★★ बदला हुआ फंक्शन: ज़ूम और रोटेट बटन के साथ नया प्लेयर UI बनाने के लिए ★★★
function showCreatorPlayerView(videoId) {
    appState.creatorPage.currentView = 'player';
    appState.creatorPage.playerScale = 1; // हर बार वीडियो खोलने पर स्केल रीसेट करें
    ...
    // ज़ूम नियंत्रण और एक शीर्ष बार कंटेनर के साथ नई HTML संरचना
        ...
        <div id="creator-page-player-container">
            <div class="player-controls-top">
                ...
                <div class="player-zoom-controls">
                    <button id="player-zoom-out-btn" class="haptic-trigger"><i class="fas fa-search-minus"></i></button>
                    <button id="player-zoom-in-btn" class="haptic-trigger"><i class="fas fa-search-plus"></i></button>
                </div>
                <button id="player-rotate-btn" class="haptic-trigger"><i class="fas fa-sync-alt"></i></button>
            </div>
    ...
    document.getElementById('player-zoom-in-btn').addEventListener('click', zoomInPlayer);
    document.getElementById('player-zoom-out-btn').addEventListener('click', zoomOutPlayer);
}
...
container.innerHTML += `<div class="claim-offer-section"> <p>इस ऑफर को पाने के लिए, अपने किसी दोस्त को 24 मिनट के अंदर इनवाइट करें।</p> <p>(To claim this offer, invite a friend within 24 minutes.)</p> <div id="claim-timer" class="reward-timer">24:00</div> <button class="continue-btn" onclick="copyToClipboard(appState.currentUser.referralCode, event)">Copy Referral ID</button> </div>`;
...
const whatsappUrl = `httpshttps://wa.me/917390928912?text=${encodeURIComponent(message)}`;
